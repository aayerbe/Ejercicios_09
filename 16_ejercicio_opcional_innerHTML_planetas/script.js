...
let planetas=[]
for (let i=0;i<3;i++){
    planetas[i]=prompt("Introduce planeta")
}

let arrayPlanetas=Object.entries(sistemaSolar)
arrayPlanetas.forEach(([key,value])=>{
    
    if(planetas.includes(value.nombre)==true){
        body.innerHTML+=`<div class="container">
                        <h1>${value.nombre}</h1>
                        <img src=${value.imagen}>
                        <p>${value.color}</p>
                        <p>${value.temperatura}</p>
                        </div>`
...
imagen.style.maxWidth="30vw"
imagen.style.maxHeight="70vh"
elemento.style.width="30vw"
elemento.style.height="70vh"
elemento.style.margin="1rem"
